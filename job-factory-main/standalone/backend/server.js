...
const USERS = [
  ...
  },
  {
    id: "admin-1",
    name: "Aarav Mehta",
    email: "admin@factoryflow.demo",
    password: "Admin#2026!",
    role: "admin",
    companyName: "FactoryFlow HQ",
  },
];
...
app.get("/api/order-drafts", requireAuth, (req, res) => {
  if (req.user.role !== "customer") return res.json({ drafts: [] });
...
app.get("/api/orders/stats", requireAuth, (req, res) => {
  ...
  const drafts =
    req.user.role === "admin"
      ...
      : store.pendingRequests.filter((draft) => draft.customerId === req.user.id);
  res.json({
    total: orders.length,
    awaitingVerification: drafts.length,
    confirmed: orders.filter((order) => ["Accepted", "Production", "Quality Check", "Completed"].includes(order.status)).length,
  });
});
...
app.post("/api/orders/confirm", requireAuth, (req, res) => {
  if (req.user.role !== "customer") return res.status(403).json({ error: "Only customers can confirm orders." });
  cleanupExpiredDrafts();
  ...
  emit(req.user.id, "order_requested", { orderId: order.orderId, status: order.status });
  for (const adminUser of getAdminUsers()) {
    emit(adminUser.id, "order_requested", { orderId: order.orderId, status: order.status, customerId: req.user.id });
  }

  res.json({
    action: "order_requested",
    reply: `${order.orderId} submitted successfully. Admin has been notified to accept, reject, or update your request.`,
    order,
  });
});

app.post("/api/admin/orders/:orderId/decision", requireAuth, requireRole("admin"), (req, res) => {
  const orderId = normalizeSpaces(req.params.orderId || "");
  const decision = normalizeSpaces(req.body?.decision || "").toLowerCase();
  const note = normalizeSpaces(req.body?.note || "");
  const statusUpdate = normalizeSpaces(req.body?.status || "");

  const order = store.orders.find((entry) => entry.orderId === orderId);
  if (!order) return res.status(404).json({ error: "Order not found." });

  const inventoryItem = store.inventory.find((entry) => entry.productName === order.productName);
  if (!inventoryItem) return res.status(400).json({ error: "Linked inventory item not found." });

  if (decision === "accept") {
    if (order.status !== "Pending Approval" && order.status !== "In Review") {
      return res.status(400).json({ error: `Order cannot be accepted from status ${order.status}.` });
    }
    if (order.quantity > inventoryItem.availableQuantity) {
      return res.status(400).json({
        error: `Insufficient stock. Only ${inventoryItem.availableQuantity} ${inventoryItem.productName} available.`,
      });
    }
    inventoryItem.availableQuantity -= order.quantity;
    order.status = "Accepted";
  } else if (decision === "reject") {
    order.status = "Rejected";
  } else if (decision === "update") {
    if (!statusUpdate) return res.status(400).json({ error: "Provide a status value when decision is update." });
    order.status = statusUpdate;
  } else {
    return res.status(400).json({ error: "Decision must be one of: accept, reject, update." });
  }
  ...
  emit(order.customerId, "order_status_changed", {
    orderId: order.orderId,
    status: order.status,
    note: order.adminNote,
  });
  emit(order.customerId, "inventory_updated", { productId: inventoryItem.id, availableQuantity: inventoryItem.availableQuantity });

  for (const adminUser of getAdminUsers()) {
    emit(adminUser.id, "order_status_changed", {
      orderId: order.orderId,
      status: order.status,
      note: order.adminNote,
    });
  ...
  res.json({
    ...
    reply: `Order ${order.orderId} updated to ${order.status}.`,
    order,
  });
});
