...
import { Factory, LockKeyhole, ShieldCheck, UserRound } from "lucide-react";
import api from "../api";
...
const DEMO_USERS = {
  ...
  admin: {
    ...
    icon: ShieldCheck,
    accent: "#c4b5fd",
    description: "Review customer requests, then accept, reject, or update order progress.",
  },
};
...
export default function LoginPanel({ onAuthenticated, onToast }) {
  ...
  const [password, setPassword] = useState(DEMO_USERS.customer.password);
  const [loading, setLoading] = useState(false);
  const [error, setError] = useState("");

  const activeDemo = DEMO_USERS[selectedRole];
      ...
      <div className="mx-auto flex min-h-screen max-w-7xl flex-col justify-center gap-8 px-6 py-10 lg:flex-row lg:items-center">
        ...
        >
          ...
          <h1 className="mt-5 text-4xl font-black leading-tight text-white sm:text-5xl">
            Secure customer and admin access for the AI manufacturing order demo.
          </h1>
          <p className="mt-4 max-w-lg text-sm leading-7 text-slate-300">
            Choose your portal. Customer users raise order requests. Admin users review those requests and send
            accept/reject/update decisions that instantly reflect in the customer workspace.
          </p>

          <div className="mt-8 grid gap-4 text-left md:grid-cols-2">
            {Object.entries(DEMO_USERS).map(([role, demo]) => (
              <button
                key={role}
                type="button"
                onClick={() => {
                ...
                className={`rounded-3xl border bg-white/5 p-5 text-left transition ${
                  selectedRole === role ? "border-white/30" : "border-white/10 hover:border-white/20"
                }`}
              >
                ...
                <h2 className="mt-4 text-lg font-bold text-white">{demo.label}</h2>
          ...
          <div className="mt-4 rounded-3xl border border-white/10 bg-white/5 p-5 text-left">
            <div className="flex items-center justify-between">
              <div
                className="flex h-11 w-11 items-center justify-center rounded-2xl border"
                style={{
                  background: `${activeDemo.accent}18`,
                  borderColor: `${activeDemo.accent}55`,
                  color: activeDemo.accent,
                }}
              >
                <activeDemo.icon size={18} />
              </div>
              <span
                className="rounded-full border px-3 py-1 text-[10px] font-semibold uppercase tracking-[0.2em]"
                style={{
                  color: activeDemo.accent,
                  borderColor: `${activeDemo.accent}55`,
                  background: `${activeDemo.accent}12`,
                }}
              >
                selected {activeDemo.role}
              </span>
            </div>
          </div>
        </motion.div>
        ...
        >
          <div className="mb-6 flex items-center gap-3">
            ...
            <div>
              ...
              <p className="text-xs text-slate-400">Protected {activeDemo.role} demo session</p>
            </div>
          </div>
